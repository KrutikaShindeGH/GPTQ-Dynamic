...
def opt_sequential(model, dataloader, dev):
    ...
    return _opt_sequential_quantize(model, dataloader, dev, optimal_bits)

@torch.no_grad()
def _opt_sequential_quantize(model, dataloader, dev, optimal_bits):
    ...
    for i in range(len(layers)):
        ...
        gptq = {}
        for name in subset:
            full_layer_name = 'model.decoder.layers.%d.%s' % (i, name)
            dynamic_wbits = optimal_bits.get(full_layer_name, args.wbits)
            
            gptq[name] = GPTQ(subset[name])
            gptq[name].quantizer = Quantizer()
            gptq[name].quantizer.configure(
                dynamic_wbits, perchannel=True, sym=args.sym, mse=False, trits=args.trits
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--target_bits', type=float, default=3.5,
        help='Target average bit-width for dynamic allocation.'
    )
    parser.add_argument(
        '--seed',
